...
public class FingerPrintTest extends AbstractE2EFunctionalTest {
	...
	public void dataPreparation() throws Exception {
		/*
		 * Create the following file structure in the same Site : In addition to the
		 * preconditions created in dataPreparation |- folder |-- pangram-banana.txt |--
		 * pangram-taco.txt |-- pangram-cat.txt |-- dog.txt
		 */
		...
		file1 = new FileModel("pangram-banana.txt", FileType.TEXT_PLAIN,
				"The quick brown fox jumps over the lazy banana");
		file2 = new FileModel("pangram-taco.txt", FileType.TEXT_PLAIN,
				"The quick brown fox jumps over the lazy dog that ate the taco");

		FileModel file3 = new FileModel("pangram-cat.txt", FileType.TEXT_PLAIN,
		...
		// Additional wait implemented to remove inconsistent failures. Ref: Search-1438
		// for details
	...
	public void makeSureFingerprintQueryWorksAfterMetadataUpdate() throws Exception {
		...
		FileModel file = new FileModel("Project_Contract.txt", FileType.TEXT_PLAIN,
				"A content which is completely different from other indexed files.");
		dataContent.usingUser(testUser).usingSite(testSite).usingResource(folder).createContent(file);

		// make sure the content has been indexed (i.e. the ContentTracker fingerprint
	...
	/**
	 * Search similar document based on document finger print. The data prep should
	 * have loaded 2 files which one is similar to the files loaded as part of this
	 * test. Note that for fingerprint to work it need a 5 word sequence.
	 */
	@Test
	public void search() {
	...
	public void searchSimilar() {
		...
		// In the response entity there is a score of each doc, change below threshold
		// to bring more like or less.
