...
public class SolrSearchInFolder extends CmisTest
{
    ...
    public void deployCustomContentModel() throws Exception
    {
        ...
        cmisApi.authenticateUser(testUser).usingSite(testSite).createFolder(parentFolder).then().usingResource(parentFolder).createFile(subFile5).and()
                .assertThat().existsInRepo().and().assertThat().contentIs("fifthFile content").createFolder(subFolder1).and().assertThat().existsInRepo()
                .createFolder(subFolder2).and().assertThat().existsInRepo().createFolder(subFolder3).and().assertThat().existsInRepo().createFile(subFile1)
                .and().assertThat().existsInRepo().createFile(subFile2).and().assertThat().existsInRepo().createFile(subFile3).and().assertThat().existsInRepo()
